...
public class ImageStreamReader {
  ...
  public List<Map<String, Object>> parsePlanesForYuvOrJpeg(@NonNull Image image) {
    ...
    for (Image.Plane plane : image.getPlanes()) {
      ...
      if (plane.getRowStride() > image.getWidth() && plane.getPixelStride() == 1) {
        // There is some padding we have to handle
        Log.d("flutter", plane.getRowStride()+"  PRIMEIRO");
        Log.d("flutter", image.getWidth()+"  SEGUNDO");
        Log.d("flutter", plane.getPixelStride()+" TERCEIRO");
        yPlaneBuffer.put("bytes", getPixels(plane.getBuffer(), image.getWidth(), image.getHeight(), plane.getRowStride(), plane.getPixelStride()));
        yPlaneBuffer.put("bytesPerRow", Math.min(plane.getRowStride(), image.getWidth()));
      } else {
